# CodeB v6.0 인프라스트럭처 가이드

> **버전: 6.0.5** | 업데이트: 2026-01-11

## 개요

CodeB v6.0은 Blue-Green 배포, 실시간 백업, 서버 마이그레이션을 지원하는 완전한 CI/CD 플랫폼입니다.
...
| 서버 | IP | 도메인 | 역할 | Podman 서비스 |
|------|-----|--------|------|---------------|
| **App** | 158.247.203.55 | app.codeb.kr, api.codeb.kr | 애플리케이션 | Next.js, Caddy, MCP API:9101, Edge Runtime, GitHub Runner |
...
└──────────────┘     └──────────────┘     └──────────────┘
       │                    │                    │
       │ 1. Build & Push    │ 2. deploy_project  │ 3. Podman
