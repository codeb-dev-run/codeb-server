...
export async function deployComposeProject(
  ...
  const {
    ...
    projectPath,
    services,
    networkName = 'podman',  // 기본 podman 네트워크 사용 (CNI 문제 방지)
  } = config;
  ...
  try {
    ...
    const actualNetwork = networkResult.networkName;
...
중요: localhost/ 이미지는 허용되지 않습니다. 반드시 ghcr.io 이미지를 사용하세요.`,
  inputSchema: z.object({
    ...
    networkName: z.string().optional().describe('Podman 네트워크 (기본: podman)'),
    ghcrAuth: z.object({
