...
export interface DeploymentResult {
  ...
  steps?: Array<{ name: string; status: string; duration?: number; message?: string }>;
  duration: number;
  url?: string;
  error?: string;
  timestamp?: string;
...
export interface HealthCheckResult {
  ...
  projectName?: string;
  environment?: Environment;
  checks: HealthCheck[] | Record<string, { status: string; responseTime?: number; message?: string }>;
  responseTime: number;
  timestamp: string;
  rollbackTriggered?: boolean;
  rollbackResult?: RollbackResult | { success: boolean; output?: string; error?: string };
}
